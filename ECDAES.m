...
const NSUInteger  kAlgorithmKeySize   = kCCKeySizeAES256;       // 32 bytes
...
+ (unsigned char *)make384Key:(NSString *)key
{
    ...
    int result = CCKeyDerivationPBKDF(kCCPBKDF2,                 // algorithm
                                      ...
                                      AESKey,                    // derivedKey
                                      kAlgorithmKeySize);        // derivedKeyLen
...
#define IV_PADI (kAlgorithmBlockSize+2)      // include 2 extra bytes, number of pad chars on the last block, plus a version number
#define CRYPT_VERSION 0                      // version number - for future use

// output clearText components:
//
// a. initialization vector (ivec) - kAlgorithmBlockSize
// b. padding size (ivec[IV_SIZE]) - 1 byte
// c. version info - 1 byte
// d. SHA1(ivec, cleartext) - kAlgorithmKeySize
// e. encrypt(cleartext)
...
+ (NSData *)encrypt:(NSData *)clear key:(unsigned char *)AESKey
{
    ...
    unsigned int padding = (clearLength % kAlgorithmBlockSize ? kAlgorithmBlockSize - clearLength % kAlgorithmBlockSize : 0);
    ...
    // fill the pad bytes 
    if (padding)
        SecRandomCopyBytes(kSecRandomDefault, (int)padding, &clearText[clearLength]);
