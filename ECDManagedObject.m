...
- (BOOL)decryptFileData:(NSData *)fileData keys:(NSArray *)keys;
{
    ...
    for (section++; section <= numSections; section++) {
        NSString *key = [keys objectAtIndex:section-1];
        if ([key isEqualToString:MODIFIEDDATEKEY]) {
            ...
            [self setPrimitiveValue:[NSString stringWithUTF8String:(const char *)str] forKey:key];
...
- (NSData *)cryptDataWithKeys: (NSArray *)keys
{
    ...
    for (partNum = 0; partNum < numSections; partNum++) {
        NSString *key = [keys objectAtIndex:partNum];
        if ([key isEqualToString:MODIFIEDDATEKEY])
    ...
    char *clearData = malloc(clearLength);
    int16_t *clearHeader = (int16_t *) clearData;
